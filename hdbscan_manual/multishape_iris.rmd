...
MCS <- 3 # minimum cluster size
```
```{r}
# import the iris data from the docker
result <- system("docker exec namenode hdfs dfs -cat ./input/multishapes.csv", intern = TRUE)
writeLines(result, "./hdbscan_manual/temp_multishapes.csv")
multishape_data <- read.csv("./hdbscan_manual/temp_multishapes.csv",
                            header = TRUE,
...
A. Do the Traversal of the internal nodes of Hieratical Tree 进行层序遍历; Since the tree is composed in a double linked format,
  ...
  if (the_Node$selfIdx > 0) { # for leaf nodes, their stability will always be 0
    ...
    if (the_Node$stability >= the_leftNode$stability + the_rightNode$stability) {
      ...
      if (the_leftNode$potentialNoise == TRUE) {
        ...
        the_leftNode$clusterBelongIdx <- the_leftNode$selfIdx # leftNode is not a potential or global noise, it will become an independent cluster with its children
      }
      ...
      if (the_rightNode$potentialNoise == TRUE) {
        ...
        the_rightNode$clusterBelongIdx <- the_rightNode$selfIdx # rightNode is not a potential or global noise, it will become an independent cluster with its children
      }
    }
  }
}
...
for (idx in 1:dp_num) {
  ...
  if (clusterSet[[a_clusterIdx]] == FALSE) {
    clusterSet[[a_clusterIdx]] <- TRUE
  }
}
