...
UnionNode <- function(targetIdx, x, y, allNode_Map) {
  ...
  # find the x and y's parent node idx
  x_parentNodeIdx <- FindNode(x, allNode_Map)
  y_parentNodeIdx <- FindNode(y, allNode_Map)

  # query out the target parent node
  x_parentNode <- query(allNode_Map, as.character(x_parentNodeIdx))
  y_parentNode <- query(allNode_Map, as.character(y_parentNodeIdx))

  x_parentNode$parentNodeIdx <- targetIdx
  y_parentNode$parentNodeIdx <- targetIdx
}
```
```{r}
...
for (idx in -1:(-dp_num + 1)) {
  ...
  UnionNode(targetIdx = idx, x = the_pointA, y = the_pointB, allNode_Map = allNode_Map)
