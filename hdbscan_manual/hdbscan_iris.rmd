...
```{r}
k_param <- 5 # the core distance of a point is the distance from the point to the k-th nearest neighbour.
...
# query(allNode_Map, "-2")
# length(query(allNode_Map, "-1")$hasChildren)
# query(allNode_Map, "40")
nodeExtractWork(-1, allNode_Map)

```
```{r}
for (idx in query(allNode_Map, "-110")$hasChildren) {
  print(idx)
}
```
```{r}
